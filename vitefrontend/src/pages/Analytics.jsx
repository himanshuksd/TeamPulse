...
export default function Analytics() {
  ...
  const completionRate = data.team_completion_rate ?? 0;
  const overdueRate = data.team_overdue_rate ?? 0;
  // ✅ FIX: tpiScore is now out of 100 (not 10) — backend fixed to return /100
  const tpiScore = data.team_tpi_score ?? 0;

  // ✅ FIX: On Track = tasks that are neither done nor overdue, expressed as % of total
  const onTrackRate = data.performance_overview?.on_track ?? Math.max(0, 100 - overdueRate - completionRate > 0 ? 100 - overdueRate : 100 - overdueRate);

  const chartBars = [
    { label: "Completion", value: Math.round(completionRate), color: "bg-blue-500" },
    { label: "Overdue", value: Math.round(overdueRate), color: "bg-red-400" },
    { label: "On Track", value: Math.round(onTrackRate), color: "bg-emerald-500" },
  ];

  // ✅ FIX: Status thresholds now based on /100 scale, not /10
  const overallStatus = tpiScore >= 70 ? "Healthy" : tpiScore >= 50 ? "Moderate" : "At Risk";
  const statusColor = tpiScore >= 70 ? "text-emerald-600" : tpiScore >= 50 ? "text-amber-600" : "text-red-500";
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
          ...
          <p className="text-3xl font-extrabold text-gray-900 tracking-tight">{completionRate}%</p>
          <div className="mt-3 w-full bg-gray-100 rounded-full h-1.5">
            <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${Math.min(completionRate, 100)}%` }} />
          </div>
          <div className="flex items-center gap-1 mt-2">
            <TrendingUp size={12} className="text-emerald-500" />
            <p className="text-xs text-emerald-600 font-semibold">On track this sprint</p>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
          ...
          <p className="text-3xl font-extrabold text-gray-900 tracking-tight">{overdueRate}%</p>
          <div className="mt-3 w-full bg-gray-100 rounded-full h-1.5">
            <div className="bg-red-400 h-1.5 rounded-full" style={{ width: `${Math.min(overdueRate, 100)}%` }} />
          </div>
          <div className="flex items-center gap-1 mt-2">
            {overdueRate > 20
              ? <><TrendingUp size={12} className="text-red-500" /><p className="text-xs text-red-500 font-semibold">Needs attention</p></>
        ...
        {/* ✅ FIX: TPI card now shows /100 and progress bar uses tpiScore directly */}
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">TPI Score</p>
            <div className="w-8 h-8 bg-emerald-50 rounded-lg flex items-center justify-center">
          ...
          <p className="text-3xl font-extrabold text-gray-900 tracking-tight">{Math.round(tpiScore)}</p>
          <div className="mt-3 w-full bg-gray-100 rounded-full h-1.5">
            {/* ✅ FIX: was tpiScore * 10 (wrong), now tpiScore directly since scale is /100 */}
            <div className="bg-emerald-500 h-1.5 rounded-full" style={{ width: `${Math.min(tpiScore, 100)}%` }} />
          </div>
          <div className="flex items-center gap-1 mt-2">
            <TrendingUp size={12} className="text-emerald-500" />
            {/* ✅ FIX: label was "Out of 10.0", now correctly "Out of 100" */}
            <p className="text-xs text-emerald-600 font-semibold">Out of 100</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5">
            ...
            <p className="text-sm text-blue-700 leading-relaxed">
              {completionRate >= 70
                ? `Great momentum! Your team is completing ${completionRate}% of tasks. Focus on reducing the ${overdueRate}% overdue rate to push TPI above ${Math.min(100, Math.round(tpiScore + 5))}.`
                : `Your completion rate of ${completionRate}% needs attention. Consider breaking large tasks into smaller ones and reassigning overdue items.`
              }
            </p>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                { label: "Tasks Completed On Time", value: `${completionRate}%`, color: "text-emerald-600" },
                { label: "Tasks Overdue", value: `${overdueRate}%`, color: "text-red-500" },
                // ✅ FIX: was "tpiScore / 10", now "tpiScore / 100"
                { label: "Team Health Score", value: `${Math.round(tpiScore)} / 100`, color: "text-blue-600" },
                { label: "Overall Status", value: overallStatus, color: statusColor },
              ].map(item => (
                <div key={item.label} className="flex items-center justify-between py-1 border-b border-gray-50 last:border-0">
                  <span className="text-sm text-gray-500">{item.label}</span>
                  <span className={`text-sm font-bold ${item.color}`}>{item.value}</span>
