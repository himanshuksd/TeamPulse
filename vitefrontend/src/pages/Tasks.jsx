...
const STATUS_STYLE = {
  DONE: { text: "text-emerald-700", bg: "bg-emerald-50", border: "border-emerald-200", label: "Done" },
  IN_PROGRESS: { text: "text-blue-700", bg: "bg-blue-50", border: "border-blue-200", label: "In Progress" },
  TODO: { text: "text-gray-600", bg: "bg-gray-100", border: "border-gray-200", label: "To Do" },
};
...
export default function Tasks() {
  ...
  const handleUpdate = async (taskId) => {
    try {
      await api.put(`/tasks/${taskId}`, { title: editText });

      setTasks(prev =>
        prev.map(t => t.id === taskId ? { ...t, title: editText } : t)
      );
  ...
  if (fetching) return <p className="text-center py-10">Loading...</p>;
  if (error) return <p className="text-center py-10 text-red-500">Error loading tasks</p>;
        ...
        {filtered.map(task => (
          <div key={task.id} className="flex items-center justify-between p-3 border-b">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-blue-500">{task.projectName}</span>
            </div>
