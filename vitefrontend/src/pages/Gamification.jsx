...
export default function Gamification() {
  ...
  // ✅ FIX: Read user from localStorage correctly
  const currentUser = (() => {
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      // GET /teams/{id}/members returns points, level, streak, total_completed from DB
      const membersRes = await api.get(`/teams/${activeTeamId}/members`);
      const members = membersRes.data;
      ...
      const board = members
        .map(m => ({
          ...
          level: m.level ?? 1,
          streak: m.streak ?? 0,
          // ✅ FIX: total_completed now comes from DB (updated when tasks are marked Done)
          total_completed: m.total_completed ?? 0,
          avatar: m.name.slice(0, 2).toUpperCase(),
        }))
        .sort((a, b) => b.points - a.points)
      ...
      // ✅ FIX: Find current user by user_id (not by name which could mismatch)
      const me = board.find(m => m.user_id === currentUser.user_id || m.user_id === currentUser.id);
      if (me) {
        const pointsInLevel = me.level * 200;         // points needed to reach next level
        const pointsPrevLevel = (me.level - 1) * 200; // points at start of current level
        const progress = me.level === 1
          ? Math.min(100, Math.round((me.points / 200) * 100))
          : Math.min(100, Math.round(((me.points - pointsPrevLevel) / (pointsInLevel - pointsPrevLevel)) * 100));
        setMyStats({ ...me, nextLevelPoints: pointsInLevel, progress });
      }
    } catch (err) {
      console.error("Failed to load gamification data:", err);
    } finally {
  ...
  // Badge computation based on myStats from DB
  const badges = BADGE_DEFS.map(b => {
    if (!myStats) return { ...b, earned: false };
    let earned = false;
    if (b.id === "first_task") earned = myStats.total_completed >= 1;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {myStats ? (
        ...
        // ✅ FIX: Show helpful message when current user not found in team members
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-sm text-amber-700">
          ⚠️ Your stats could not be loaded. Make sure you are a member of the active team and have completed some tasks.
        </div>
      )}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-bold text-gray-900">Level {myStats.level} → Level {myStats.level + 1}</p>
                <p className="text-xs text-gray-400">{myStats.points} / {myStats.nextLevelPoints} points</p>
              </div>
            </div>
            <span className="text-sm font-extrabold text-blue-600">{myStats.progress}%</span>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-3">
            <div
              className="bg-gradient-to-r from-blue-500 to-blue-600 h-3 rounded-full transition-all relative overflow-hidden"
              style={{ width: `${myStats.progress}%` }}
            >
              <div className="absolute inset-0 bg-white/20 animate-pulse rounded-full" />
            </div>
          ...
          <p className="text-xs text-gray-400 mt-2">
            {myStats.nextLevelPoints - myStats.points} points to reach Level {myStats.level + 1}
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {leaderboard.length > 0 && (
            <div className="grid grid-cols-3 gap-4">
              {leaderboard.slice(0, 3).map(member => {
                const RankIcon = RANK_ICON[member.rank] || Medal;
                const isMe = member.user_id === (currentUser.user_id || currentUser.id);
                return (
                  <div key={member.rank} className={`bg-white border-2 rounded-2xl p-5 text-center shadow-sm hover:shadow-md transition-all ${isMe ? "border-blue-400" :
                      member.rank === 1 ? "border-yellow-200" :
                        member.rank === 2 ? "border-gray-200" : "border-orange-200"
                    }`}>
                    <div className="flex justify-center mb-3">
                      <div className="relative">
                        <div className="w-14 h-14 rounded-2xl bg-blue-500 flex items-center justify-center text-white text-lg font-extrabold">
            ...
            {leaderboard.length === 0 ? (
              ...
              <div className="divide-y divide-gray-50">
                {leaderboard.map(member => {
                  ...
                  return (
                    <div key={member.rank} className={`flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors ${isMe ? "bg-blue-50" : ""}`}>
                      <span className={`text-sm font-extrabold w-5 text-center ${member.rank === 1 ? "text-yellow-500" :
                          member.rank === 2 ? "text-gray-400" :
                            member.rank === 3 ? "text-orange-400" : "text-gray-400"
                        }`}>{member.rank}</span>
                      <div className="w-9 h-9 rounded-xl bg-blue-500 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                        {member.avatar}
                      </div>
                      <div className="flex-1">
                        ...
                        {/* ✅ FIX: Shows actual task count from DB */}
                        <p className="text-xs text-gray-400">{member.total_completed} tasks · Level {member.level}</p>
                      </div>
                      <div className="flex items-center gap-1">
                        <Flame size={12} className="text-red-400" />
